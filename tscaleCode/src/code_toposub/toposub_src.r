...
stack_raster<-function(demLoc,predNames, predRoot, predFormat){
	require(rgdal)
	require(raster)
	
	#make gridmaps file
	gridmaps <- readGDAL(demLoc)
	names(gridmaps)[1] <- 'ele'
...
#create dir for rasters (fuzzy membership)
	...
	for(c in (1:Nclust)){
		...
		c=formatC(c,width=2,digits = 0,flag="0", format="f")#bug fix
		writeRaster(x,paste(rstdir,"/tmp_", c, sep=""), overwrite=T) #membership rasters
	}
}
...
spatial<-function(col=col, esPath=esPath, format='.tiff', Nclust=Nclust){
	...
	writeRaster(fuzRst, paste(esPath, '/result/out/fuz_',col, format, sep=''),NAflag=-9999, overwrite=T)
#endScript
}
...
crispSpatial_noinform<-function(col,Nclust, esPath, landform){
	...
	as.vector(meanX$x)->meanX
		...
		writeRaster(rst, paste(esPath, '/result/crispRst_noinform/',col,'_',l,'.asc', sep=''),overwrite=T)
...
crispSpatial<-function(col,Nclust, esPath, landform){
		...
		data.frame(seq,meanX)->meanXdf
		subs(land, meanXdf,by=1, which=2)->rst
		writeRaster(rst, paste(esPath,'/result/out/crisp_',col,'_',l,'.tif', sep=''),overwrite=T)
